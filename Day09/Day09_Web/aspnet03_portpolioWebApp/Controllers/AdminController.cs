    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> CreateRole(CreateRoleModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _roleManager.CreateAsync(role); //저장된 권한명이 DB에 저장
        ...
        {
            ViewData["NoScroll"] = "true";  // 게시판은 메인스크롤이 안생김
            var role = await _roleManager.FindByIdAsync(id); //  aspentroles 테이블에서 id 조회
            if (role == null)
            {
        ...
        {
            var role = await _roleManager.FindByIdAsync(model.Id); //  aspentroles 테이블에서 id 조회
            if (role == null)
            {
                TempData["error"] = $"권한이 없습니다.";
                return NotFound();
        ...
        public async Task<IActionResult> EditUsersInRole(List<UserRoleModel> model, string roleId)
        {
            ...
            foreach (var user in model)
            {
                ...
                {
                    result = await _userManager.AddToRoleAsync(userObj, role.Name); //권한을 사용자에게 할당
                }
                else if (!user.IsSelected && await _userManager.IsInRoleAsync(userObj, role.Name))
                {
                    result = await _userManager.RemoveFromRoleAsync(userObj, role.Name); // 권한을 사용자에게 삭제
                }
            }
